...
class WOODManager(ASerial):
    ...
    def disconnect(self) -> None:
        ...
        self.medium.close_connection(None)
        self.connected = False
        if self.__eventlistener is not None:
            thread_running = False
            self.__eventlistener.join()
            self.__eventlistener = None
    ...
    def connect(self, event_handler: Union[None, callable] = None) -> bool:
        ...
        if not self.medium.connected:
            ...
            print("connection not started medium is already connected")
        ...
        if self.uses_socket and self.__eventlistener is None:
            ...
            self.__eventlistener = Thread(
                target=receive_events,
                args=(self, self.medium, self.__event_handler),
                daemon=True,
            )
            thread_running = True
    ...
    def send_proxies(self, config: dict) -> None:
        ...
        sers = encoder.serialize_proxies(
            Interrupts["recvproxies"], h, p, lst, self.max_bytes
        )
        msgs = []
...
def receive_events(wood: WOODManager, aMedium: AMedium, callback: callable) -> None:
    ...
    while thread_running:
        try:
            ...
            dbgprint(f"exception occured at Thread {e}")
            break
...
def wood_state_to_wa_state(dump_json: dict) -> dict:
    ...
    _frame_types = {0: "fun", 1: "init_exp", 2: "block", 3: "loop", 4: "if"}
...
def wa_state_to_wood_state(_json: dict, offset: str) -> dict:
    ...
    _frame_types = {"fun": 0, "init_exp": 1, "block": 2, "loop": 3, "if": 4}
