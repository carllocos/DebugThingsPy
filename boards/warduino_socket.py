...
#     @property
#     def step_state(self):
#         return (self.__stepdump, self.__stemplocals)
...
#         ask4commit = AMessage(Interrupts['updateModule'] + '\n', receive_done)
#         sers = encoder.serialize_wasm(wasm, self.max_bytes)
#         l = len(sers)
#         msgs = [ask4commit]
#         for idx, content in enumerate(sers):
#             rpl = receive_done if (idx + 1) == l else receive_ack
#             msgs.append(
#                 AMessage(Interrupts['updateModule'] + content + '\n',  rpl))
...
#     def callstack_msgs(self, state=None):
#         dbgprint('generating callstack_msgs')
#         dump_msg = AMessage(Interrupts['dump'] + '\n', receive_dump)
#         locs_msg = AMessage(Interrupts['locals'] + '\n', receive_locals)
#         return [dump_msg, locs_msg]
