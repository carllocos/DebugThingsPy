...
logger = log.get_logger("OutOfThings/BinaryProtocol")
END_MSG = " \n"
MAX_BYTES = 1000
...
def encode_state(dssession, max_bytes=None):
    ...
    quanty_bytes_header = (
        1 + 4
    )  # 1 byte for interrupt (= 2 hexa chars) & 4 bytes for quantity bytes send (= 8 hexa chars)
...
def int2bytes(i, quantity_bytes, byteorder="big"):
    signed = True if i < 0 else False
    return (i).to_bytes(quantity_bytes, byteorder, signed=signed)  # 'little'
...
def serialize_stackvalues(vals, chunks, max_space):
    ...
    def add_chunk():
        ...
        assert (
            len(chunks[-1]) <= max_space
        ), f"failed for chunk idx {len(chunks) -1 } #{len(chunks[-1])}"
...
def serialize_callstack(callstack, chunks, max_space):
    ...
    for frame in callstack:
        ...
        if max_space < len(current_chunk) + header_len + len(frames_ser) + len(
            frame_ser
        ):
            if frames_ser != "":
...
def serialize_brtable(br_table, chunks, max_space):
    ...
    elems_ser = list(
        map(lambda v: int2bytes(v, quantity_bytes=4).hex(), br_table["labels"])
    )
...
def valtype2int(t):
    ...
    i = types.get(t)
    if i is None:
        logger.error(f"Unknwon stack value type {t}")
        exit(-1)
    return i


def val2bytes(value_type, val):
    logger.debug(f"serializing value {val} with type {value_type}")
