...
def generate_dbginfo(path: Union[PurePath, str]) -> DBGInfo:
    """
    ...
    fname = filepath.name[: len(ext) -1]# remove extension 

    #TODO generate debug information
    parent_path = "/".join(filepath.parts[:-1]) #TODO change name for generic sol
    tmp_dir =  PurePath(parent_path) 
    headers_path = tmp_dir.joinpath(fname + '.dbg.headers')
    details_path = tmp_dir.joinpath(fname + '.dbg.details')
    srcmap_path = tmp_dir.joinpath(fname +  '.dbg.verbose.txt')
