...
class Functions:
    ...
    def exports(self) -> List[Function]:
        return list(
            filter(lambda f: f.export_name is not None, self.__int2funcs.values())
        )

    @property
    def imports(self) -> List[Function]:
        return list(
            filter(lambda f: f.import_name is not None, self.__int2funcs.values())
        )
    ...
    def from_dbg(dbg_info: DBGInfo, codes: Codes, types: Types):
        ...
        for f in mod["funcs"]:
            ...
            funcs.append(
                Function(idx, sign, name, export_name, import_name, _fun_locals, code)
            )
